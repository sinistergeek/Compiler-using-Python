...
class Parser:
    def __init__(self,tokens:"list[Token]"):
        self.tokens = tokens
        self.index = 0
        self.current_token: Options[Token] = None
        self.advance()
    def advance(self):
    ...
    def expression(self):
        result = self.term()
        while self.current_token is not None and self.current_token.type in (tokens.TT-PLUS,tokens.TT_MINUS):
            if self.current_token.type == tokens.TT_PLUS:
                ...
                self.advance()
                result = BinaryNone(tokens.TT_MINUS,result,self.term())
        return result
    def term(self):
        result = self.factor()
